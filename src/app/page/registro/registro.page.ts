...
export class RegistroPage {
  ...
  async registrar(perfilUsuario: PerfilUsuario) {
    if (
      ...
      perfilUsuario.user.password === '' ||
      perfilUsuario.user.pass === '' ||
      ...
      console.log(
        'Nombre de usuario es demaisado largo (máximo de 20 caracteres)'
      );
      const alert = await this.alertController.create({
        header: 'ERROR',
        message:
      ...
      console.log(
        'Nombre de usuario es demaisado corto (mínimo de 4 caracteres)'
      );
      const alert = await this.alertController.create({
        header: 'ERROR',
        message:
      ...
      perfilUsuario.correo.includes('@') == false ||
      perfilUsuario.correo.includes('.') == false ||
      /[a-zA-Z]/.test(perfilUsuario.correo) == false
    ) {
